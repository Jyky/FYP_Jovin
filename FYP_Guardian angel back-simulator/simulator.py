        ...
        for row in range(height):
              for col in range(width):
                        if floorplanRGB[col,row][0:3] == (255,255,255): #white
                                ...
                                floorplan[col][row] = 0
                        elif floorplanRGB[col,row][0:3] == (34,177,76): #green
                                floorplan[col][row] = 2
                        else:   #other color
                                floorplan[col][row] = 3
        del floorplanRGB
                ...
                for row in range(height):
                        if floorplan[col][row] == 2:
                                ...
                                if r-row != 0 or c-col !=0:
                                        cur.execute('''INSERT OR IGNORE INTO Toilet (x, y, height, width) 
                                                                   VALUES ( ?, ?, ?, ? )''', ( midC, midR, r-row, c-col) ) #save midpoint of the square into DB
                                        conn.commit()
...
def movement(q, floorplan, toilets, elderlies, elderly, id):
        ...
        def wander():
                ...
                if distance(elderly.x, elderly.y, elderly.lastUpdateX, elderly.lastUpdateY)>5:
